...
LINKAGE_BANDGAP: Dict[str, Tuple[float, float, float]] = {
    ...
    "imide":           (2.5,   0.5,  -0.05),   # C=O slightly electron-withdrawing
...
def estimate_gas_uptake(
    ...
    if gas == "CO2" and temperature_K >= 280 and pressure_bar <= 2.0:
        ...
        # CO2/N2 selectivity: depends on both linkage AND pore size
        # Larger pores → weaker confinement → lower selectivity
        # Calibrated from GCMC: small pore (PLD<5Å) ~35-50×, large pore (PLD>12Å) ~8-18×
        base_sel = {"imine": 35, "beta_ketoenamine": 25, "boronate_ester": 18,
                    "hydrazone": 30, "triazine": 45, "<UNK>": 28}.get(linkage_type, 28)
        # Pore size correction: selectivity halves for every ~8Å increase in PLD
        pore_correction = max(0.25, 1.0 - max(0, pld_angstrom - 5.0) * 0.06)
        sel = float(np.clip(base_sel * pore_correction, 4.0, 80.0))

        return {"uptake_mmol_g": round(u, 3), "co2_n2_selectivity": round(sel, 1),
...
LINKAGE_MODULUS: Dict[str, Tuple[float, float]] = {
    ...
    "imide":            (18.0, 4.0),   # 5-membered imide ring, PI-film analogy
    "olefin":           (12.0, 4.0),
    "<UNK>":            (10.0, 3.0),
}
...
LINKAGE_STABILITY: Dict[str, Dict] = {
    ...
    "imide": {
        # PI-COFs: TGA >500°C, excellent chemical stability in water, acid, base
        # Irreversible bond — not hydrolysed under normal lab conditions
        "thermal_decomp_C": (500, 40),
        "water_stability":  "excellent",
        "acid_stability":   "good",
        "base_stability":   "moderate",   # hot base can cleave imide ring
        "hydrolysis_t12_h": 5000,
    },
}

def predict_stability(linkage_type: str, node_bb: str = "", linker_bb: str = "") -> Dict:
    """
    Predict COF stability from linkage type and building block identity.
    BB-specific corrections calibrated from TGA/hydrolysis literature.
    """
    ...
    water   = info["water_stability"]
    acid    = info["acid_stability"]
    base    = info["base_stability"]
    t12     = info["hydrolysis_t12_h"]

    # BB-specific thermal stability corrections (from TGA literature)
    # Porphyrin and phthalocyanine macrocycles are inherently more thermally stable
    BB_THERMAL_CORRECTION = {
        "S4_PORPH": +40,   # metalloporphyrin COFs decompose at ~380-420°C
        "S4_PHTH":  +50,   # phthalocyanine COFs very stable, >400°C
        "T3_TPA":   +10,   # triphenylamine backbone more stable than benzene
        "T3_TPM":   +5,
        "T3_TRIZ":  +20,   # triazine core itself very stable
    }
    T_mean += BB_THERMAL_CORRECTION.get(node_bb, 0)

    # BB-specific water stability corrections
    # Large macrocycles are harder to hydrolyse (kinetic protection)
    if node_bb in ("S4_PORPH", "S4_PHTH") and linkage_type == "imine":
        # Porphyrin imine COFs have improved hydrolytic stability vs plain imine
        # due to macrocycle planarity constraining the imine geometry
        water  = "moderate"   # same, but upgrade if currently poor
        t12   *= 3            # ~144 h vs 48 h for plain imine
        if acid == "very_poor":
            acid = "poor"

    # Linker-specific: azobenzene and triazole linkers add hydrolytic stability
    if linker_bb in ("L2_AZBN", "L2_BTTA"):
        t12 = int(t12 * 1.5)
    ...
    return {
        ...
        "acid_stability":             acid,
        "base_stability":             base,
        "hydrolysis_t12_hours":       t12,
        "stability_score":            _stability_score({"water_stability": water,
                                                         "acid_stability":  acid,
                                                         "base_stability":  base}),
    }
...
def compute_full_properties(
    ...
    pref = predict_preferred_stacking(a, b, n_arom, spec.linkage_type,
                                       node_bb=spec.node_bb, linker_bb=spec.linker_bb)
    ...
    # Adsorption — pass pld so selectivity scales with pore size
    ...
    # Stability — pass BB names for BB-specific corrections
    stab = predict_stability(spec.linkage_type, spec.node_bb, spec.linker_bb)
