...
def test_imide_linkage():
    """Imide COFs: PI-COF-1/2/3 validated properties."""
    from decoder.reticular_decoder import BB_LIBRARY, COFSpec
    from decoder.validity_checker import synthesizability_score
    from analysis.pxrd_simulator import predict_preferred_stacking
    from analysis.property_predictor import predict_stability, estimate_band_gap

    # 1. Imide is in BB_LIBRARY
    assert "imide" in BB_LIBRARY["conn_groups"], "imide missing from conn_groups"
    nf, lf = BB_LIBRARY["conn_groups"]["imide"]
    assert nf == "NH2" and lf == "anhydride", f"Wrong imide func groups: {nf},{lf}"

    # 2. Dianhydride linkers exist
    for linker in ("L2_PMDA", "L2_NTCDA", "L2_BTDA"):
        assert linker in BB_LIBRARY["L2_linkers"], f"{linker} missing from L2_linkers"

    # 3. Synthesizability: imide should be >0.55 (well-established chemistry)
    s = synthesizability_score("imide", "T3_BENZ", "L2_PMDA", "hcb")
    assert s > 0.55, f"Imide synth score too low: {s:.3f}"

    # 4. Stacking: imide should prefer AB more than imine (dipole repulsion)
    prefs_imide = predict_preferred_stacking(22.0, 22.0, 50, "imide",
                                              node_bb="T3_BENZ", linker_bb="L2_PMDA")
    prefs_imine = predict_preferred_stacking(22.0, 22.0, 50, "imine",
                                              node_bb="T3_BENZ", linker_bb="L2_BENZ")
    assert prefs_imide["AB"] > prefs_imine["AB"], \
        f"Imide should have higher AB than imine: {prefs_imide['AB']:.3f} vs {prefs_imine['AB']:.3f}"

    # 5. Thermal stability: imide >480°C
    stab = predict_stability("imide", "T3_BENZ", "L2_PMDA")
    assert stab["thermal_decomp_T_C"] >= 480, \
        f"Imide thermal stability too low: {stab['thermal_decomp_T_C']}°C"
    assert stab["water_stability"] == "excellent", \
        f"Imide water stability should be excellent, got: {stab['water_stability']}"

    # 6. Band gap: imide ~2.3-2.7 eV (wider than imine due to C=O EWG)
    bg, unc = estimate_band_gap("imide", "T3_BENZ", "L2_PMDA", 0.3, 50)
    assert 1.5 <= bg <= 3.5, f"Imide band gap out of range: {bg:.2f} eV"

check("imide linkage — PI-COF properties", test_imide_linkage)

def test_imide_cif_generation():
    """Imide COF CIF has correct unit cell vs PI-COF-1 literature (a=24.5 Å)."""
    import tempfile
    from pathlib import Path
    from data.synthetic_cif_generator import generate_hcb_cif

    cif = generate_hcb_cif("T3_BENZ", "L2_PMDA", "imide", "NH2", "anhydride")
    # Extract cell parameter a
    a_cif = None
    for line in cif.splitlines():
        if line.startswith("_cell_length_a"):
            a_cif = float(line.split()[1])
            break
    assert a_cif is not None, "CIF missing _cell_length_a"
    # PI-COF-1 literature: a = 24.5 Å, allow ±2 Å
    assert abs(a_cif - 24.5) <= 2.0, \
        f"PI-COF-1 cell parameter wrong: {a_cif:.2f} Å (expect ~24.5 Å)"

check("imide CIF generation (PI-COF-1 cell)", test_imide_cif_generation)

def test_porphyrin_properties_distinct():
    """S4_PORPH + L2_PYRN should differ from S4_PORPH + L2_BIPH in stacking and stability."""
    from analysis.pxrd_simulator import predict_preferred_stacking
    from analysis.property_predictor import predict_stability

    p1 = predict_preferred_stacking(34.5, 34.5, 72, "imine",
                                     node_bb="S4_PORPH", linker_bb="L2_PYRN")
    p2 = predict_preferred_stacking(34.5, 34.5, 50, "imine",
                                     node_bb="T3_BENZ", linker_bb="L2_BENZ")
    # Porphyrin should have higher AA preference than small benzene system
    assert p1["AA"] > p2["AA"], \
        f"Porphyrin AA% ({p1['AA']:.3f}) should exceed T3_BENZ ({p2['AA']:.3f})"

    # S4_PORPH thermal correction: should be +40°C vs plain imine
    stab_porph = predict_stability("imine", "S4_PORPH", "L2_PYRN")
    stab_plain = predict_stability("imine", "T3_BENZ", "L2_BENZ")
    assert stab_porph["thermal_decomp_T_C"] > stab_plain["thermal_decomp_T_C"], \
        f"Porphyrin Td ({stab_porph['thermal_decomp_T_C']}) should exceed T3_BENZ ({stab_plain['thermal_decomp_T_C']})"

check("porphyrin properties distinct from small-node COFs", test_porphyrin_properties_distinct)
