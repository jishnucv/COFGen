...
COMMERCIAL_LINKERS = {
    ...
    "L2_PYRN", "L2_STIL",
    # Dianhydride linkers — all commercially available (Sigma, TCI, Alfa Aesar)
    "L2_PMDA", "L2_NTCDA", "L2_BTDA",
}
EASY_LINKAGES = {"imine", "boronate_ester", "beta_ketoenamine"}

# Linker-specific solubility / handling penalties
# Large or insoluble linkers reduce synthesizability even if the chemistry is easy
LINKER_SYNTH_PENALTY = {
    "L2_TPHN":  0.10,   # terphenyl: limited solubility, needs high-boiling solvent
    "L2_BTTA":  0.05,   # triazole linker: moderate synthetic complexity
    "L2_NTCDA": 0.05,   # slightly less soluble than PMDA
    "L2_BTDA":  0.05,   # benzophenone dianhydride: moderate solubility
}
# Node-specific penalties
NODE_SYNTH_PENALTY = {
    "T3_TRIF":  0.08,   # trifluoromethyl: more complex synthesis
    "T3_INTZ":  0.10,   # interspersed triazine: multi-step synthesis
    "S4_PHTH":  0.05,   # phthalocyanine: more expensive/complex
}
...
def synthesizability_score(
    ...
    Accounts for linkage chemistry, commercial availability, topology precedent,
    and linker/node-specific solubility and handling penalties.
    Imide COFs score moderately (0.6–0.75): chemistry is well-established but
    requires high-temperature solvothermal conditions without acid catalyst.
    """
    score = 0.0
    ...
    if linkage in EASY_LINKAGES:
        score += 0.40
    elif linkage in {"hydrazone", "imide"}:
        score += 0.30   # well-established, higher than triazine
    elif linkage == "triazine":
        score += 0.18   # requires ionothermal or superacid conditions
    else:
        score += 0.10
    ...
    # Penalise for linker/node difficulty
    score -= LINKER_SYNTH_PENALTY.get(linker_bb, 0.0)
    score -= NODE_SYNTH_PENALTY.get(node_bb, 0.0)
